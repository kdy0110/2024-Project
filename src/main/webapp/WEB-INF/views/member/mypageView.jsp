            ...
            <div class="container">
                ...
                <div class="row justify-content-center">
                    <div class="col-lg-8 col-xl-7">
                        <form method="post" action="<c:url value="/updateDo" />"> <!-- /updateDo 이걸로 지정해줘야 마이페이지 수정 가능  -->
                            <div class="form-floating mb-3">
                                <input readOnly value="${sessionScope.login.memId}" class="form-control" name="memId" id="memId" type="text" 
                                placeholder="아이디를 입력해주세요" data-sb-validations="required" />
                                <label for="memId">아이디</label>
                                <div class="invalid-feedback" data-sb-feedback="name:required">A ID is required.</div>
                            </div>
                            <div class="form-floating mb-3">
                                <input class="form-control" value="${sessionScope.login.memNm }" id="memNm" name="memNm" type="text" 
                                placeholder="이름을 입력해주세요" />
                                <label for="memNm">이름</label>
                            </div>
                            <div class="form-floating mb-3">
                                <input class="form-control" value="${sessionScope.login.memKg }" id="memKg" name="memKg" type="text" 
                                placeholder="몸무게를 입력해주세요" data-sb-validations="required" />
                                <label for="memKg">몸무게</label>
                            </div>
                            <div class="form-floating mb-3">
                                <input class="form-control" value="${sessionScope.login.memCm }" id="memCm" name="memCm" type="text" 
                                placeholder="키를 입력해주세요" />
                                <label for="memCm">키</label>	
                            </div>
                            <button class="btn btn-primary btn-xl mb-5" id="submitButton" type="submit">수정하기</button>
                        </form>
                    </div>
                </div>
            </div>
